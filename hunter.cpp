...
Hunter::Hunter(const int hunterId, const NenAbility& type, const int aura, const int fightsHad) :
    hunterId(hunterId), nenType(type), aura(aura), fightsHad(fightsHad), parent(0), hunter_nen(nenType), sub_group_nen(nenType) {};
...
void Hunter::SetFightsHad(int fights) {
...
std::shared_ptr<Squad> Hunter::GetSquad() {
    return squad.lock();
}
